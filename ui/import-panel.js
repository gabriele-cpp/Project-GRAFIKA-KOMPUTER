...
<aside id="import-panel">
  ...
  <div class="ip-section">
    ...
    <div class="ip-upload-zone" id="ip-upload-zone">
      ...
      <div class="ip-upload-formats">GLTF · GLB · OBJ</div>
    </div>
    <input type="file" id="ip-file-input" accept=".gltf,.glb,.obj" style="display:none">
    ...
    <div class="ip-section">
      ...
      <div class="ip-field">
        ...
        <div class="ip-slider-row">
          <input type="range" id="ip-slider-count" min="1" max="100" step="1" value="1">
          <input type="number" class="ip-num" id="ip-num-count" min="1" max="100" step="1" value="1">
        </div>
      </div>
    </div>
...
export class ImportPanel {
    ...
    _handleFile(file) {
        ...
        if (!['gltf','glb','obj'].includes(ext)) {
            this.callbacks.onError?.(`Format .${ext} tidak didukung. Gunakan GLTF, GLB, atau OBJ.`);
            return;
        }
        this.setProgress(0);
        this.callbacks.onUpload?.(file);
    }
    ...
    _bindControls() {
        ...
        this._syncPair('ip-slider-count', 'ip-num-count', 'ip-val-count',
            ...
            1, 100, 1, v => Math.round(v).toString()
        );
    ...
    _resetSliders() {
        ...
        if (countSlider) countSlider.value = 1;
        if (countNum)    countNum.value    = 1;
